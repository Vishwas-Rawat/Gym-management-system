...
public interface MemberRepository extends JpaRepository<Member, Integer> {
  ...
  // Get active members of a gym who are either not assigned to any trainer OR
  // already assigned to THIS trainer
  @Query("SELECT m FROM Member m WHERE m.gym.gymId = :gymId AND m.isActive = true AND m.deletedAt IS NULL AND (m.trainer IS NULL OR m.trainer.trainerId = :trainerId)")
  List<Member> findMembersForTrainerAssignment(@Param("gymId") Long gymId, @Param("trainerId") Integer trainerId);
